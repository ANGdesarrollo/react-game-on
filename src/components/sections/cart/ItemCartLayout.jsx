            ...
            {con.itemAdded.map(el => {
                ...
                return (
                    <div key={el.id} className='cart-product'>
                        ...
                        <div className='cart-product__total'><h2>{`TOTAL: $${(el.price * el.qty).toFixed(2)}`}</h2></div>
                        <div className='cart-product__delete'><img onClick={()=> {con.deleteItem(el.id).toFixed(2)}} src="https://res.cloudinary.com/dwz16rstr/image/upload/v1662958505/react-js-game-on/icons/delete_h4swr7.png" alt=""/></div>
                    </div>
                )
            })}
            <div className='total-to-pay'>
                <div><h2>SUBTOTAL: ${con.subTotalToPay()} </h2></div>
                <div><h2><span>TAX 21%: </span>${con.subTotalToPay() * 0.21}</h2></div>
                <div><h2>TOTAL: ${con.subTotalToPay() + con.subTotalToPay() * 0.21}</h2></div>
